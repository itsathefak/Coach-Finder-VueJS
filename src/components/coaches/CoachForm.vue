<template>
  <form @submit.prevent="submitForm">
    ...
    <p v-if="!formIsValid">Please fix the above errors and submmit again</p>
...
export default {
  ...
  methods: {
    ...
    validateForm() {
      ...
      if (!this.firstName.isValid) this.formIsValid = false;

      // Last name validation
      this.lastName.isValid = this.lastName.val.trim() !== '';
      if (!this.lastName.isValid) this.formIsValid = false;

      // Description validation
      this.description.isValid = this.description.val.trim() !== '';
      if (!this.description.isValid) this.formIsValid = false;

      // Hourly Rate validation
      this.rate.isValid = this.rate.val && this.rate.val > 0;
      if (!this.rate.isValid) this.formIsValid = false;

      // Areas of expertise validation
      this.areas.isValid = this.areas.val.length > 0;
      if (!this.areas.isValid) this.formIsValid = false;
    },
